...
set.seed(72)
dlnn.model<-mlp(residual_arima,
                hd=c(10,12),
                difforder = 0,outplot = TRUE,
                retrain = TRUE,allow.det.season = FALSE,
                reps = 1,
                lags = 1:2,
                sel.lag = FALSE)
...
residual_dlnn<-flow_data_xts-fit_dlnn

residual_dlnn_xreg<-ts.intersect(residual_dlnn,split_data(flow_data_xts_xreg,20)$train)

arimax_indiv<-Arima(residual_dlnn_xreg[,1],
                    xreg = residual_dlnn_xreg[,2],
...
raw_dlnn_arimax_train<-ts.intersect(flow_data_xts,fit_dlnn+fit_arimax)
...
raw_dlnn_arimax<-ts.intersect(flow_data_xts,fit_frc_dlnn,fit_frc_arimax)
