...
set.seed(72)
ffnnx.model<-mlp(residual_arima,
                 hd=c(1),
...
residual_ffnnx<-flow_data_xts-fit_ffnnx

arima_indiv<-Arima(residual_ffnnx,
...
# raw_ffnnx_arima_train<-ts.intersect(flow_data_xts,fit_ffnnx+fit_arima)
# TSrepr::mape(raw_dlnn_arimax_train[,1],raw_dlnn_arimax_train[,2])
# TSrepr::rmse(raw_dlnn_arimax_train[,1],raw_dlnn_arimax_train[,2])
# raw_dlnn_arimax_test<-ts.intersect(flow_data_xts,frc_dlnn+frc_arimax)
# TSrepr::mape(raw_dlnn_arimax_test[,1],raw_dlnn_arimax_test[,2])
# TSrepr::rmse(raw_dlnn_arimax_test[,1],raw_dlnn_arimax_test[,2])

raw_ffnnx_arima<-ts.intersect(flow_data_xts,fit_frc_ffnnx,fit_frc_arima)
#raw_dlnn_arimax<-raw_dlnn_arimax %>% data.frame()
colnames(raw_ffnnx_arima)<-c("Outflow","DLNN","ARIMAX")

raw_ffnnx_arima%>%data.frame()%>%
  cbind(date=index(raw_ffnnx_arima)%>%yearmon()) %>%
  mutate(Predicted=(DLNN+ARIMAX)/1000,
...
for(h in c(1:24))
{
  frc_ffnnx<-forecast(ffnnx.model,h=h,xreg = as.data.frame(flow_data_xts_xreg))$mean
  frc_arima<-forecast(arima_indiv,h=h)$mean
  
  intersect_data<-ts.intersect(frc_ffnnx+frc_arima,
