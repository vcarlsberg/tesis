...
for(location in c("Jakarta"))
{
  for(denomination in c("K100000","K50000","K20000","K10000","K5000","K2000","K1000","L1000","L500","L200","L100","L50"))
  {
    ...
    mlp.model<-mlp(as.ts(residual),hd=c(9),reps=1,
    ...
    compile<-rbind(compile,data.frame(Model="ARIMAX-MLPX_Seri",
                                      InOutSample="In Sample",
                                      Location=location,
                                      Denomination=denomination,
                                      fh=0,
                                      MAPE=mape(result[,1],result[,3]+result[,4])))
    ...
    for (fh in 1:24)
    {
      ...
      result.pred<-ts.union(train_test_data$test[1:fh,1],
